...
export default function AppMiddleware({
  ...
  // DEMO MODE: Skip encryption for local demo
  const demoMode = process.env.REACT_APP_DEMO_MODE === 'true' || !process.env.REACT_APP_FIREBASE_API_KEY;

  // Auto-create demo user if in demo mode and no user exists
  useEffect(() => {
    if (demoMode && !localStorage.getItem('demoUser')) {
      const demoUser = {
        email: 'demo@local.user',
  ...
  const getAndSaveSecretKey = useCallback(async (uid: string) => {
    if (demoMode) {
      // Auto-set a demo key
      setSecretKeyToCache('Demo1234')

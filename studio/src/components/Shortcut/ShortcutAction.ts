...
export type CreateNewPadShortcutHandler = () => void | Promise<void>

let createNewPadShortcutHandler: CreateNewPadShortcutHandler | null = null

export function setCreateNewPadShortcutHandler(handler: CreateNewPadShortcutHandler | null) {
  createNewPadShortcutHandler = handler
}
  ...
  if (pressed) {
    ...
    if (pressed.shift && pressed.control && pressed.b) {
      ...
      setSettingState(
        produce<ISettingStore>((state) => {
          ...
          // Only resize window if in Tauri app (dynamic import to avoid loading Tauri SDK in browser)
    ...
    if (pressed.control && pressed.s) {
      ...
      ctrlSTimeout = setTimeout(() => {
        message.info("😎 canis.studio auto-saves your work")
      }, 250) as unknown as number
      return
    }

    // New pad (immediate; no modal)
    if (pressed.control && pressed.n) {
      preventEvent(ev)
      if (createNewPadShortcutHandler) {
        void Promise.resolve(createNewPadShortcutHandler()).catch(console.error)
      }
    }

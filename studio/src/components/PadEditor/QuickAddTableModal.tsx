    ...
    <Popover.Panel className="absolute w-44 z-10 mt-3 px-4 sm:px-0 dropdown translate-y-9 right-0">
              ...
              <tr key={rw} className={'row'}>
                {columns.map((cln) => {
                  ...
                  return (
                    <td
                      key={cln}
                      onMouseMove={() => onMouseMove(rw, cln)}
                      onClick={() => {
                        editor?.chain().focus().insertTable({ rows: rw, cols: cln, withHeaderRow: true }).run()
                      }}
                      className={`${isMoving && startMouseMove.current && 'mouse-moving'} ${isSelected && 'bg-zinc-400'} column cursor-pointer`}
                    ></td>
                  )
                })}
              </tr>
      ...
      <div className="table-input">
        <div className="flex justify-center my-4 form-control">
          ...
          {open && <button onClick={() => {
            setColumn(prev => ({ ...prev, selectedColumn: column.selectedColumnInput }))
            setRow(prev => ({ ...prev, selectedRow: row.selectedRowInput }))
            editor?.chain().focus().insertTable({ rows: row.selectedRowInput, cols: column.selectedColumnInput, withHeaderRow: true }).run()
            setOpen(false)
          }} className="btn w-9 h-full undefined px-1.5 py-1.5 btn-primary">OK</button>}
        </div>
      </div>
    </Popover.Panel>
  )
